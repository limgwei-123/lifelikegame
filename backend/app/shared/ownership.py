...
def get_owned_goal_or_raise(goal_repo:GoalRepository , goal_id, user_id):
  goal = goal_repo.get_by_id_and_user_id(goal_id, user_id)
  if not goal:
...
def get_owned_task_or_raise(task_repo: TaskRepository, task_id, user_id):
  task = task_repo.get_by_id_and_user_id(task_id, user_id)
  if not task:
...
def get_owned_task_schedule_or_raise(task_schedule_repo: TaskScheduleRepository, task_schedule_id, user_id):
  task_schedule = task_schedule_repo.get_by_id_and_user_id(task_schedule_id, user_id)
...
def get_owned_task_instance_or_raise(task_instance_repo: TaskInstanceRepository, task_instance_id, user_id):
  task_instance = task_instance_repo.get_by_id_and_user_id(task_instance_id, user_id)
...
def get_owned_reward_or_raise(reward_repo: RewardRepository, reward_id, user_id):
  reward = reward_repo.get_by_id_and_user_id(reward_id=reward_id, user_id=user_id)
  if not reward:
