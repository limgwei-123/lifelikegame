...
@router.post("/tasks/{task_id}/task_schedules/{task_schedule_id}/task_instances", response_model=TaskInstanceResponse, status_code=status.HTTP_201_OK)
def create_task_instance_for_date(task_id,task_schedule_id, payload: CreateTaskInstanceRequest, current_user = Depends(get_current_user), task_instance_service:TaskInstanceServiceInterface = Depends(get_task_instance_service)):
  task_instance = task_instance_service.create_task_instance_for_date(task_id=task_id, task_schedule_id=task_schedule_id, payload=payload, user_id=current_user.id)
